...
**Fix:** Widen to `name[1024]` (other NTFS implementations uses this), or return variable-length with a caller-owned buffer.
...
- fs-ntfs doesn't spawn threads internally; in-process concurrency
  isn't a hazard.
- External concurrency (Windows / NTFS driver) can't be prevented by

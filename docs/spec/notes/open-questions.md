...
- [x] (2026-05-03) `MFT_RECORD_IS_VIEW_INDEX` set on host record for every view index in §4.13. **RESOLVED**: src/mkfs.rs:169-171 emits VIEW_INDEX (0x0008) on $ObjId (rec 16), $Reparse (rec 17), and $Quota (rec 18); corroborated by `docs/chkdsk-improvement-findings.md §2.2.3`. All pass chkdsk.
...
- [x] (2026-05-25) `COLLATION_NTOFS_ULONG` (0x10) used by `$Quota:$Q` — **RESOLVED**: `src/mkfs.rs:59` emits this value for `$Secure:$SII` (per [MS-FSCC §2.4]); also used by `$Quota:$Q`. Confirmed by 42/42 chkdsk passes.
- [x] (2026-05-03) `COLLATION_NTOFS_SID` (0x11) used by `$Quota\$O`. **RESOLVED**: src/mkfs.rs:1304 emits COLLATION_NTOFS_SID for $Quota:$O (citing MS-FSCC §2.4). Confirmed by 42/42 chkdsk passes.
...
- [x] (2026-05-03) `$Extend` directory vs file structural shape. **RESOLVED**: $Extend is a directory with $I30. The chkdsk /scan exit-13 ceiling was caused by missing VIEW_INDEX flags on child records (S4-v1 bug), not the directory shape. Fixed in staging-4 (2026-05-27, PR #52); 42/42 matrix passes.
...
- [ ] (2026-09-22) `$EA_INFORMATION`'s `u16` packed length, `u16` NEED_EA count, `u32` query length order is corroborated by ntfs-3g and Linux ntfs3, but the fixture set has no Windows-authored EA summary — proposed test: have Windows write two EAs with exactly one NEED_EA flag, then record the raw eight-byte value.

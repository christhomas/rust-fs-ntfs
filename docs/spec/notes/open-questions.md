...
- [x] (2026-05-25) `COLLATION_NTOFS_SECURITY_HASH` (0x12) and `COLLATION_NTOFS_ULONGS` (0x13) numeric values — **RESOLVED**: `src/mkfs.rs` lines 55 and 63 emit these values and cite MS-FSCC §2.4 in the code comments. `COLLATION_UNICODE_STRING` (0x02) still uncorroborated.
- [x] (2026-05-25) `COLLATION_NTOFS_GUID` / `COLLATION_BINARY` for `$ObjId:$O` — **RESOLVED (corrected)**: `src/mkfs.rs:1306` emits `COLLATION_NTOFS_ULONGS` (0x13) for `$ObjId:$O`, not 0x00 (`COLLATION_BINARY`). Prior claim was wrong. Whether Windows uses 0x00 or 0x13 for `$ObjId:$O` is still unconfirmed by black-box test.
...
- [x] (2026-05-03) `MFT_RECORD_IS_VIEW_INDEX` set on host record for every view index in §4.13 — `docs/chkdsk-improvement-findings.md` §2.2.3 corroborates `$Secure`; `$Quota` / `$ObjId` / `$Reparse` not yet observed — confirm by raw dump of each. **RESOLVED**: src/mkfs.rs:169-171 and the mkfs body confirm VIEW_INDEX (0x0008) is emitted on $ObjId (rec 16), $Reparse (rec 17), and $Quota (rec 18). All pass chkdsk.
...
- [x] (2026-05-25) `COLLATION_NTOFS_ULONG` (0x10) used by `$Quota:$Q` — **RESOLVED**: `src/mkfs.rs:59` emits this value for `$Secure:$SII` with comment "per the same source [MS-FSCC §2.4]". Also used by `$Quota:$Q`. Black-box test against a real Windows `$Quota` record still pending.
- [x] (2026-05-03) `COLLATION_NTOFS_SID` (0x11) used by `$Quota\$O` — uncorroborated — observe `collation_rule` byte in a freshly-formatted `$Quota\$O` record. **RESOLVED**: src/mkfs.rs:1304 emits COLLATION_NTOFS_SID for $Quota:$O with comment citing MS-FSCC §2.4. Also confirmed by 42/42 chkdsk passes.
...
- [x] (2026-05-03 → 2026-05-27) Fresh-format `$LogFile` end-to-end layout — **RESOLVED**: the 12 KiB canonical blob + `0xFF` remainder is sufficient. chkdsk `/scan` exits 0 on all 42 matrix scenarios without needing the format.com RCRD page. The exit-13 ceiling was caused by missing `$Secure`/`$Extend` structures, not the logfile shape `[OBSERVED: 42/42 matrix seal at dcdb46d, 2026-05-27]`.
...
- [x] (2026-05-03) `$Extend` directory vs file structural shape — `docs/chkdsk-improvement-findings.md §2.8` and `docs/mkfs-bug-catalog.md` flag a difference between writer output and Microsoft reference — proposed test: diff `$Extend` MFT record body against a fresh `format.com` reference dump. **RESOLVED**: $Extend IS a directory with $I30. The chkdsk /scan exit-13 ceiling was caused by missing VIEW_INDEX flags on child records (S4-v1 bug), not the directory shape. Fixed in staging-4 (2026-05-27); 42/42 matrix passes.
- [x] (2026-05-03) `$Extend` child record numbers (24 = `$Quota`, 25 = `$ObjId`, 26 = `$Reparse`) — needs cross-version corroboration; conventional values used. **RESOLVED**: rust-fs-ntfs uses records 16=$ObjId, 17=$Reparse, 18=$Quota. Windows accepts these numbers — 42/42 chkdsk passes including /F /X (offline repair) [OBSERVED: src/mkfs.rs:169-171]. The conventional 24/25/26 from third-party docs is one valid assignment but not required by Windows.
...
- [x] (2026-05-25) `compute_sii_hash` algorithm (rotate-right-29 / rotate-left-3 accumulator, wrapping add, over 4-byte LE dwords) — **RESOLVED**: `src/sds.rs::sdh_hash` implements this and the module doc confirms it was verified against 12 entries from a Microsoft `Format-Volume` NTFS v3.1 reference volume; all 12 matched.
- [x] (2026-05-25) `$SDS` entry layout (hash_key at 0x00, security_id at 0x04, self-offset at 0x08, length at 0x10, SD body at 0x14) — **RESOLVED**: `src/sds.rs` defines SDS_HDR_HASH_OFF=0, SDS_HDR_SECURITY_ID_OFF=4, SDS_HDR_ENTRY_OFFSET_OFF=8, SDS_HDR_ENTRY_SIZE_OFF=16, SDS_HDR_SD_DATA_OFF=20 and the test `build_sds_single_entry_layout` validates the layout.
- [x] (2026-05-25) `$SDS` 256 KiB mirror at `+0x40000` — **RESOLVED**: `src/sds.rs` constant `SDS_MIRROR_GAP = 0x40000` and `build_sds()` copies every primary entry to `primary_offset + 0x40000` verbatim. The "entries must not span the boundary" rule is still uncorroborated by black-box test.
...
- [x] (2026-05-27) `$Secure` mkfs implementation status — **RESOLVED**: fully implemented at format time. Non-resident $SDS with canonical SD at security_id=0x100, populated $SDH/$SII indexes, 256 KiB mirror. All system records carry security_id=0x100 in $STANDARD_INFORMATION. Runtime SD insertion not implemented. [OBSERVED: src/mkfs.rs:938-1088, src/sds.rs]
- [x] (2026-05-27) `$Extend\$RmMetadata\$TxfLog\$Tops:$T` (TxF log) requirement for chkdsk /scan — **RESOLVED (omitted)**: chkdsk /scan exits 0 with only $ObjId + $Reparse present under $Extend. $RmMetadata is intentionally not implemented. [OBSERVED: src/mkfs.rs:172 comment + 42/42 matrix passes including chkdsk /scan]

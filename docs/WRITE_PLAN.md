...
- **No GPL code consulted.** Use MS-FSCC, Flatcap, and upstream `ntfs`
  (MIT/Apache-2.0) only. `NTFS driver` (GPL) is off-limits. If an
...
- **Tests against real disk images.** Fixtures live in `test-disks/`
  ...
  qemu+Alpine VM running `NTFS formatter` + `NTFS driver` mount-for-populate). The
...
### W1.3 — `set_file_attributes` / file attribute tools (✅)
...
**Known limitation.** `NTFS formatter` lays out root directories with
...
docs (Flatcap, MS-FSCC, upstream ntfs) — never at `NTFS driver`.

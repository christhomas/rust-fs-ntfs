...
| Script | What it does |
|---|---|
| `scripts/run-matrix.sh` | Underlying harness wrapper (`../fs-windows-test-harness`). Filters by substring. |
...
- The Windows VM provisioning (IP, SSH keys, work directory) — that's
  in `../fs-windows-test-harness/scripts/setup-local.sh` and the
  `.test-env` file.
- The harness internals — see `../fs-windows-test-harness/README.md`.

...
#
# The list below is build-ntfs-feature-images.sh's build_* functions. The two
# Windows-only fixtures have their own PowerShell builder and are checked when
# the integration gate sets NTFS_FIXTURES_REQUIRED=1.
set -euo pipefail
...
if [ -n "${NTFS_FIXTURES_REQUIRED:-}" ]; then
    for img in attrlist compressed; do
        [ -f "$dir/ntfs-$img.img" ] || missing="$missing ntfs-$img.img"
    done
fi
if [ -n "$missing" ]; then
